...
for name, pins in sorted(slice_inputs):
    ...
    for p in pins:
        ...
        add_direct(slice_interconnect_xml, input_name, 'BLK_BB-%s.%s' % (slice_type, input_name.split('.')[-1]))
...
for name, pins in sorted(slice_outputs):
    ...
    for p in pins:
        ...
        add_direct(slice_interconnect_xml, ('BLK_BB-%s.%s' % (slice_type, output_name.split('.')[-1])), output_name)
